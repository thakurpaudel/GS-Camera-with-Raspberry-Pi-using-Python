...
def display_images(original, compressed, timestamp, compression_ratio, frame_rate):
    ...
    info_text = f"Timestamp: {timestamp.strftime('%Y-%m-%d %H:%M:%S')}, Compression Ratio: {compression_ratio:.2f}:1, FPS: {frame_rate:.2f}"

    # Draw information on images
    original_image = draw_info(original_image, f"Original - {info_text}")
    compressed_image = draw_info(compressed_image, f"Compressed - {info_text}")
...
def capture_images(queue):
    ...
    while True:
        ...
        compression_ratio = len(original_image_data) / len(compressed_image_data)
        ...
        queue.put((original_image_data, compressed_image_data, timestamp, compression_ratio, frame_rate))
...
if __name__ == "__main__":
    ...
    try:
        while True:
            original, compressed, timestamp, compression_ratio, frame_rate = image_queue.get()
